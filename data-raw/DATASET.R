...
process_slp_sheet <- function(path, sheet, range, col_names, watt_multiplier = 1) {
...
cols_2025 <- do.call(paste, c(expand.grid(days, tolower(month.name)), sep = "_"))
...
load_profiles_2025 <- lapply(sheets_2025, function(sheet) {
  process_slp_sheet(path_2025, sheet, "B4:AL100", cols_2025, watt_multiplier = 4)
})
...
# build info lists
infos_DE <- setNames(
  lapply(profiles, function(p) list(profile = p, description = description_DE[[p]], details = details_DE[[p]])),
  profiles
)
infos_EN <- setNames(
  lapply(profiles, function(p) list(profile = p, description = description_EN[[p]], details = details_EN[[p]])),
  profiles
)
