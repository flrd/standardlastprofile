...
slp_plot_1999 <- function() {
  ...
  ggplot2::ggplot(
    ...
    ggplot2::scale_y_continuous(NULL, breaks = c(0, 200, 400), limits = c(0, NA), labels = \(x) paste(x, "W")) +
    ggplot2::scale_color_manual(
      name   = NULL,
      values = c("winter" = "#961BFA", "summer" = "#FA9529", "transition" = "#0CC792")
    ) +
    ggplot2::labs(
...
slp_plot_2025 <- function(months = c("december", "march", "july")) {
  ...
  tmp <- slp[slp$profile_id %in% c("H25", "G25", "L25", "P25", "S25") &
