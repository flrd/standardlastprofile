...
support planning and optimising electricity generation and distribution.
...
SLP. The value for `"00:00"` indicates the average power consumed
between 00:00 and 00:15. The `slp_electricity_profiles` dataset contains
...
week.](slp-electricity_files/figure-html/small_multiples_vignette-1.png)
...
- The **1999 profiles** (`H0`, `G0`–`G6`, `L0`–`L2`) were published as
  ...
  all 15-minute intervals equals 1,000 kWh.[^3]
...
3.  Use result from 2nd step to extract values from
    `slp_electricity_profiles`.[^5]
...
seasonal periods) available in the dataset `slp_electricity_profiles`.
...
normalised to an annual consumption of 1,000
kWh.](slp-electricity_files/figure-html/G5_plot_vignette-1.png)

As you can see, the values in 2023 for December 24 (a Sunday) and
December 25 and 26 (both public holidays) are identical.

### Special cases: H0, H25, P25, S25
...
in the `slp_electricity_profiles` dataset for `H0`, `H25`, `P25`, and
...
\\ w_d = w_s \times(-3.92\mathrm{e}{-10} \times d^4 + 3.20\mathrm{e}{-7}
\times d^3 - 7.02\mathrm{e}{-5} \times d^2 + 2.10\mathrm{e}{-3} \times
d + 1.24) \\ Where:
...
versa.](slp-electricity_files/figure-html/H0_2026_plot-1.png)
...
[^5]: That is actually a lie. There is an internal data object from
