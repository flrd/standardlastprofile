...
nationwide in Germany as Sundays. State-level holidays are excluded because
they vary by state and can change. Use the `holidays` argument to supply your
own dates — the built-in data are then ignored entirely.
...
The final chart below places the 2025 household profiles side by side against
`H0` as a reference, showing how cumulative energy consumption diverges over
the course of a year. The grey line is `H0`; the coloured line is the 2025
profile in each panel.

```{r H0_vs_2025, echo=FALSE, message=FALSE, fig.asp=0.5, fig.retina=2, out.width="100%"}
...
setDT(dat)
dat[, kWh := (cumsum(watts) - watts[1]) / 4 / 1000, by = profile_id]

idx_H0 <- dat[, profile_id == "H0"]
dat_H0 <- dat[idx_H0]
dat_H25_P25_S25 <- dat[!idx_H0]

ggplot(dat_H25_P25_S25, aes(start_time, kWh)) +
  geom_line(
    data = dat_H0[, .(start_time, kWh)],
    colour = "#CCCCCC"
  ) +
  geom_line(aes(colour = profile_id), linewidth = 1) +
  geom_pointless(aes(colour = profile_id), size = 2) +
  facet_grid(cols = vars(profile_id)) +
  scale_x_datetime(NULL, date_labels = "%b") +
  scale_y_continuous(NULL, breaks = c(0, 500, 1000), labels = \(x) paste(x, "kWh")) +
  scale_colour_manual(
    name = NULL,
    values = c(
      "H25" = "#961BFA",
  ...
  labs(
    title = "Cumulative energy consumption: H0 vs. H25, S25, and P25",
    subtitle = "Reference line = H0; based on 1,000 kWh/a",
    caption = "data: www.bdew.de"
  ) +
  theme_minimal() +
  theme(
