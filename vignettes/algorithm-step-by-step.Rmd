...
The study found that consumption varies both over the course of the year and over the days within a week. The definition of `period`s is:

* `summer`: May 15 to September 14
* `winter`: November 1 to March 20
* `transition`: March 21 to May 14, and September 15 to October 31

It was also found that there was no significant difference in consumption on weekdays from Monday to Friday in any group. For this reason, the days Monday to Friday are grouped together as 'workdays'. December 24th and 31th are considered a Saturday too, if they are not a Sunday, and public holidays are set to a Sunday.
...
There is an optional argument `state_code` that can take one of 16 values representing a German state. This allows you to consider holidays that are defined at the state level. Call `german_states` to get an overview of the possible values and their meaning:

```{r german_states, message=FALSE}
german_states
```
...
data.frame(input = date_seq, day = wkday_period)
```
...
Use data at your own caution. Feel free to [create an issues](https://github.com/flrd/standardlastprofile/issues) and suggest imporvements.
