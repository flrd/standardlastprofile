...
There are 96 x 1/4 hour measurements of electrical power for each unique combination of `profile_id`, `period` and `day`, which we refer to as the "standard load profile". The value for "00:00" indicates the average work done in the morning between 00:00 and 00:15. 
The data was collected and analyzed in 1999 by the Association of the Electricity Industry[^vdew-1] (VDEW) on the basis of 1,209 measured load profiles of low-voltage customers in Germany[^bdew-2] and is provided by German Association of Energy and Water Industries (BDEW Bundesverband der Energie- und Wasserwirtschaft e.V.).
  ...
  [^vdew-1]: VDEW, which once represented approximately 750 German energy supply companies, advocated for their interests with both politicians and consumers. In 2007, VDEW merged into the BDEW.
...
ggplot(slp,
  ...
  scale_y_continuous(NULL, n.breaks = 3) +
...
$$
w = w_0 \times(-3.92\mathrm{e}{-10} \times d^4 + 3.20\mathrm{e}{-7} \times  d^3 - 7.02\mathrm{e}{-5} \times d^2 + 2.10\mathrm{e}{-3} \times d + 1.24)
$$
Where:

* $w$ is the resulting 'dynamic' quarter-hour value
* $w_0$ is the static quarter-hourly value
...
profile.Finally, the following chart compares the static values with their
dynamic counterparts.[^bdew-3]
...
```{r H0_dynamic, echo=FALSE, message=FALSE, fig.retina=2}
# helper
paste_dash <- function(...) paste(..., sep = "-")
lst <- list(
  winter = paste_dash("1997-01", 17:19),
  summer = paste_dash("1996-07", 19:21),
  transition = paste_dash("1997-04", 18:20)
)
...
ggplot(H0_plot,
  ...
  labs(title = "Standard Load Profile 'H0' - dynamic versus static",
       subtitle = "96 x 1/4h measurements a day, based on consumption of 1,000 kWh/a",
       caption = "data: www.bdew.de") +
  theme_minimal() +
  theme(legend.position = "top") +
  theme(strip.text.y.right = element_text(angle = 0)) + 
...
Use data with caution. Feel free to 
