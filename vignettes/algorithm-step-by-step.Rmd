...
$$
x = x_0 (-3.92e^{-10} \times d^4 + 3.20^{-7} \times  d^3 - 7.02^{-5} \times d^2 + 2.10^{-3} \times d + 1.24)
$$
Where:
...
``` {r H0_2023_daily, message=FALSE, echo=FALSE, fig.retina=2, fig.asp=0.5}
# aggregate by day of year as decimal number (1 - 365)
H0_2023_daily <- by(H0_2023, INDICES = format(H0_2023$start_time, "%j"), FUN = function(x) {
  data.frame(
...
``` {r H0_2023_plot, message=FALSE, echo=FALSE, fig.retina=2, fig.asp=0.5}
...
ggplot(H0_2023_daily, aes(start_time, watts)) +
  geom_line(color = "#0CC792") +
